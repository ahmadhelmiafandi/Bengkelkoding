            ...
            <form action="{{ route('polis.update', $poli->id) }}" method="POST">
                ...
                <div class="flex gap-3">
                    <button type="submit"
                        ...
                        class="px-6 py-2.5 rounded-lg bg-primary-blue hover:bg-[#1e2d6b] 
                               text-white font-semibold text-sm transition shadow-md">
                        <i class="fas fa-save mr-1"></i> Simpan
                    </button>

                    <a href="{{ route('polis.index') }}"
                        class="px-6 py-2.5 rounded-lg bg-slate-100 hover:bg-slate-200 
                               text-slate-600 font-semibold text-sm transition border border-slate-200">
                        Batal
                    </a>
                </div>
